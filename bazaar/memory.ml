...
type args = { prefix : string; modules : (module AssetLibrary) list }
type state = args

let rec strip_leading_slash path =
  if String.starts_with ~prefix:"/" path then
    strip_leading_slash (String.sub path 1 (String.length path - 1))
  else path
...
let call (conn : Conn.t) t =
  ...
  if String.starts_with ~prefix:t.prefix rel_path then
